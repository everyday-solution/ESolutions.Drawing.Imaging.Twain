...
namespace ESolutions.Drawing.Imaging.Twain
{
	...
	internal class InternalTwainController : System.Windows.Forms.IMessageFilter
	{
		...
		public InternalTwainController (IntPtr parentControl)
		{
			parentWindowHandle = parentControl;
			Initialize ();
		}
		#endregion
		...
		public void Acquire ()
		{
			...
			{
				dataSource.Enable (parentWindowHandle);
			}
			else
			{
				dataSource.Close ();
		...
		private void Initialize ()
		{
			...
			if (this.dataSourceManager.Open (ref parentWindowHandle) == ReturnCodes.Success)
			{
		...
		private void TransferPictures ()
		{
			...
			do
			{
				...
				if ((rc = newImageInfo.Get ()) == ReturnCodes.Success)
				{
					...
					System.Drawing.Image image = ESolutions.Win32.Gdi32.DibToBitmap (dibPointer);
